...
class Boo_Tools
{
    ...
    {
        Zend_Registry::get('log')->debug($url);
        $curl = curl_init();
    ...
    //goo.gl token
    static protected function googlToken($b)
    {
        $i = self::tke($b);
        $i = $i >> 2 & 1073741823;
        $i = $i >> 4 & 67108800 | $i & 63;
    ...
    static protected function tkd($l)
    {
    ...
    static protected function tke($l)
    {
        $m = 5381;
    ...
    static protected function tkf($l)
    {
        $m = 0;
        for($o = 0; $o < strlen($l); $o++){
